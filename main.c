...
Frame resize_max(Frame frame,const int fitter,const int stride){//池化的窗口大小和步长
    ...
    for(int i=0;i<output.height;i++){//i标记行数
        ...
        //遍历图像
            ...
            output.data[i*output.linesize+j*3]=MAX_R;
            output.data[i*output.linesize+j*3+1]=MAX_G;
...
//命令行编写
    ...
    argparse_option options[] = {
        ...
        XBOX_ARG_INT(&frame_skipping,"-s","--skip","fram skip",NULL,"skip"),
        
        XBOX_ARG_BOOLEAN(NULL,"-r","--resize","resize by pooling", NULL, "resize"),
        XBOX_ARG_INTS_GROUP(&resize,NULL,NULL,"information for resizing",NULL,"inf_r"),
        //调整池化窗口大小（默认fitter=2，stride=2）
        
        XBOX_ARG_STR(&filepath, "-f","--file","filepath",NULL, "filepath"),
        //选择要播放的文件的路径（含默认值）       
    ...
    if (XBOX_ismatch(&parser, "resize")){
        ...
        int Is_legal = XBOX_ismatch(&parser, "inf_r");
        if(Is_legal!=2){
            printf("不合法输入");
            return -1;
        }
...
//解码视频
    ...
    while(1){
        ...
        sleep(2*((double) (end - start)) / CLOCKS_PER_SEC);
